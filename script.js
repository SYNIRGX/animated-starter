...
const styleMeta = {
  brutalism: ['HARD EDGES', 'RAW MATERIAL', 'DIRECT IMPACT'], maximalism: ['COLOR STACK', 'VISUAL EXCESS', 'MORE IS MORE'], minimalism: ['NEGATIVE SPACE', 'QUIET SYSTEMS', 'LESS / BETTER'], glassmorphism: ['LIGHT LAYERS', 'DEPTH + BLUR', 'TRANSPARENT UI'], skeuomorphism: ['TACTILE UI', 'FAMILIAR OBJECTS', 'PRESS / FEEL'], neobrutalism: ['BOLD BORDERS', 'FRIENDLY CHAOS', 'UTILITY + JOY'], y2k: ['CHROME FUTURE', 'PIXEL MEMORY', 'LOADING...'], editorial: ['TYPE AS VOICE', 'VISUAL RHYTHM', 'CULTURE / FORM'], organic: ['SOFT SYSTEMS', 'NATURAL MOTION', 'GROW / FLOW'], cyberpunk: ['SIGNAL / NOISE', 'ACCESS GRANTED', 'BREAK THE GRID'], aurora: ['LIGHT FIELD', 'SLOW WONDER', 'DREAM IN COLOR'], index: ['12 LANGUAGES', 'ONE CANVAS', 'CHOOSE A MOOD']
};

document.querySelectorAll('.style-section').forEach((section) => {
  const meta = styleMeta[section.dataset.style];
  const grid = section.querySelector('.section-grid');
  if (!grid || !meta) return;
  const rail = document.createElement('div');
  rail.className = 'style-detail-rail reveal';
  rail.innerHTML = meta.map((item, index) => `<span><b>0${index + 1}</b>${item}</span>`).join('');
  section.appendChild(rail);
  const trace = document.createElement('div');
  trace.className = 'style-trace';
  trace.innerHTML = `<span>${section.dataset.style.toUpperCase()} / VISUAL STUDY</span><i></i><span>SCROLL TO OBSERVE</span>`;
  section.appendChild(trace);
});

const visualFixes = document.createElement('style');
visualFixes.textContent = `
  /* Keep the custom cursor as the only pointer visible on desktop. */
  html, body, body *, body *::before, body *::after { cursor: none !important; }

  /* Preserve the established heading position while giving the composition a wider horizontal axis. */
  .style-section .section-grid { grid-template-columns: 9% minmax(0, 1fr) minmax(340px, 42%); column-gap: 3vw; align-items: center; }
  .style-header { position: relative; z-index: 8; min-width: 0; max-width: 760px; }
  ...
  /* Body copy sits directly below the heading and reads as a horizontal text block. */
  .style-header p:not(.kicker) { position: relative; z-index: 9; width: min(100%, 560px); max-width: 560px; margin: 0 0 28px; color: #34343a; font-size: 16px; line-height: 1.45; text-wrap: pretty; }
  .brutalism .style-header p:not(.kicker), .maximalism .style-header p:not(.kicker), .neobrutalism .style-header p:not(.kicker) { color: #241b1b; }
  .minimalism .style-header p:not(.kicker), .editorial .style-header p:not(.kicker), .skeuomorphism .style-header p:not(.kicker) { color: #4b4843; }
  .glassmorphism .style-header p:not(.kicker), .y2k .style-header p:not(.kicker), .cyberpunk .style-header p:not(.kicker), .aurora .style-header p:not(.kicker) { color: rgba(255,255,255,.9); text-shadow: 0 2px 16px rgba(0,0,0,.45); }
  .organic .style-header p:not(.kicker) { color: #214f40; }

  /* All card/device artwork occupies the right column instead of competing with the heading. */
  .style-header + * { grid-column: 3; justify-self: end; margin-left: auto; }
  .brute-poster, .glass-card, .skeuo-device, .neo-card, .y2k-object, .editorial-page, .organic-note, .cyber-terminal, .aurora-card, .minimal-object, .max-collage, .style-list { margin-right: 0; }
  ...
  @media (max-width: 800px) {
    ...
    .style-header p:not(.kicker) { width: 100%; max-width: 34rem; font-size: 15px; }
    .style-header + * { margin-left: auto; margin-right: auto; }
